...
**9.21 Reviewed against Anthropic's published Claude Code guidance.** The review turned up one real weakness and several smaller ones.

The weakness: acceptance criteria were prose a human reads. Anthropic's headline advice is that an agent stops when work looks done, and that without a check it can run, "looks done" is the only signal available and the user becomes the verification loop. So every stage now ships an executable check under `verify/`, and those scripts persist afterwards as a security regression suite rather than being discarded as build scaffolding. Each property they test is one that can silently stop being true after an unrelated change months later.

Smaller changes, all from the same guidance: one session per stage, because context is the binding constraint and eight stages in one session would spend its budget on finished work; plan mode named on the stages where a wrong approach is expensive; adversarial review by a subagent on the isolation claims and the rebuild package, since the session that built a boundary is the wrong thing to attest that it holds; the Stage 4 intake reframed as the documented interview pattern using `AskUserQuestion` followed by a fresh session to act on the result; an explicit tool allowlist on unattended runs; and `WORKING.md` added to carry all of it, so `BUILD.md` stays about outcomes.
