...
|---|---|---|
| `--bare` in any script | `01` section 7, `02` stage 3 | Demands an API key, so spend leaves the subscription, and every instruction file, skill and hook is silently dropped |
| Hardcoded `ANTHROPIC_API_KEY` | `01` section 7 | Turns a fixed monthly cost into an unbounded one |
| `.mcp.json` present | `02` stage 3 | In a `-p` session, servers connect with no trust dialog and no prompt |
| `--settings` outside the exceptions file | `02` stage 3, stage 7 | Routes around every deny rule and every hook in one flag. Exactly one legitimate use exists and it is listed in `settings-exceptions.txt` |
| `--dangerously-skip-permissions` | `02` stage 3 | No explanation needed |
| `bypassPermissions` on the interactive session | `01` section 3 | The one mode that turns the interactive path into an unbounded one |
| Filename-versioned files | `01` section 4 | How a clean structure rots into an archaeological dig |
| Secrets in the rebuild package | `02` stage 6 | The package is only safe unencrypted if it is actually clean |
| Run records containing model output | `02` stage 8 | Personal content accumulating for ever inside the backup |
| **Hooks absent or not registered** | `01` section 3.1, `02` stage 3 | The enforcement layer this version added. Absent is the failure |
| **`conf/` missing from the backup** | `02` stage 6 | A restore that returns a machine which has forgotten everything it learned |
| **Agent uid can rewrite `settings.json`** | `01` section 3, `02` stage 2 | The load-bearing one. If the agent can rewrite its own deny rules and hook registrations, both are decoration |
...
- A **positive control**: a hand-built correct tree. Expect PASS, all twelve traps ok.
...
validated this way against six sabotages; this version adds four more traps and the controls
